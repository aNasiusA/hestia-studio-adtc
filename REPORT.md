...
| Peak RSS | 3,835.22 MB (~3.84 GB) |
| Steady-state RSS | 3,292 MB |
| CPU utilization (p99) | 58.5% |
| Thermal throttling | Not observed (`throttled: false`) |

These are self-reported *development-machine* benchmarks (`environment.measured_on: participant_laptop` in `submission.json`), captured on Apple Silicon rather than the literal Intel/AMD ADTC reference laptop — the run quoted in the video demo and the Devpost story. (A later re-run for a final consistency pass came in close but not identical — 31.47 tok/s, 3,715 MB peak — normal benchmark variance; kept this run as the one-of-record so the number spoken in the video, this table, and the Devpost story all agree.) Official throughput/RAM/thermal scores are produced by the ADTC profiler on the standard evaluation machine at judging time. Notes when comparing:

- **Self-reported Sperf/Seff, and the formula used:** ADTC's own FAQ confirms the Sperf/Seff fields on the Devpost form are self-computed from your own `submission.json` — there is no way to compute the *true* Sperf at self-report time regardless, since the challenge's published formula (`S_perf = 100 × TPS_actual / TPS_max`) defines `TPS_max` as "the highest speed across all submissions," a number only ADTC can know once every team has submitted. We used the reference formula the profiler tool itself documents instead (`S_perf = min(TPS / 15.0, 1.0) × 100`, `S_eff = max(0, (7.0 − peak_rss_GB) / 7.0) × 100`, decimal GB = MB ÷ 1000 throughout, applied consistently): **Sperf = 100.0** (30.33 ÷ 15.0 exceeds 1.0, capped), **Seff = 45.2** — both entered on Devpost's Additional Info form.
- Peak RSS (~3.84 GB) already leaves meaningful headroom under the ~7 GB memory budget even on this proxy machine; CPU-only inference on the actual reference laptop (no Metal/unified-memory acceleration) should not increase memory pressure, since llama.cpp's CPU backend allocates comparably or more conservatively than the Metal backend used here.
