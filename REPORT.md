...
- **Runtime:** llama.cpp exclusively, per ADTC's mandated runtime — no Ollama, no llama-cpp-python server wrapper for the graded path. We additionally verified this model against MAKO's **actual, unmodified orchestrator code** (not a mock, not a hand-written prompt imitating it): `llama-server` was started on this exact `.gguf` file, MAKO's `v3-langraph` `.env` was pointed at it (`KG_LLM_PROVIDER=openai`, `KG_BASE_URL=http://localhost:8811/v1`, `KG_EMBED_PROVIDER=local`), and `python -m orchestrator.run "A 54-year-old patient presents with chest pain..."` was run end-to-end. It completed in ~6 seconds, fully offline: selected an entry agent from the knowledge graph, made a real hop decision validated against a graph edge (`delegatesTo`, accepted on its 3rd `validate_decision` attempt after two rejected proposals), and voluntarily terminated when it judged the case needed more input. The full unedited log and trace JSON are in [`mako-integration/`](mako-integration/README.md) in this repo.
